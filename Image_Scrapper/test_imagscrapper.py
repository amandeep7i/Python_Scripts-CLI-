...
path = 'images/Datasets_ImageScrapping/4jan/'
...
output_directory = f"D:/Learning/Ml Intern(analysed)/images/Datasets_ImageScrapping/4jan/{Title}"

# Comment the below code from 59-83 if renaming is not completed and also delete the files howing error 
while n_images != 0 :
...
# Specify the path that is in your local system 
  ...
  try:
    ...
    try:
      ...
      # cmd = f"""wget -A jpg "{image}"""
      # os.system(cmd)
      wget.download(image,out = output_directory)
    except:
      var+=1
      continue
    # imgcnt+=1
  ...
  var += 1
  n_images -= 1
# Function to convert the downloaded Images and rename them
imgcnt=1
for img in glob.iglob(f'{output_directory}/*'):
  im = Image.open(img)
# converting to jpg
  rgb_im = im.convert("RGB")
# exporting the image
  rgb_im.save(f"{output_directory}/{Title}_{imgcnt}.jpg")
  os.remove(img)
  imgcnt+=1
